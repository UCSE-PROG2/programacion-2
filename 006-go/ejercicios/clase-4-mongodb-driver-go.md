# Ejercicios — Clase 4: MongoDB en profundidad

Ejercicios de evaluación para la [Clase 4](../README.md#clase-4--mongodb-en-profundidad). Parten de `producto.MongoRepository` ya conectado desde la Clase 2, y retoman con operadores reales de Mongo varios casos que en la Clase 5 se resolvieron "a mano" trayendo todo a memoria.
...
3. Si existe `InMemoryRepository` (Clase 6, Ejercicio 6, opcional), implementarlo también ahí, recorriendo el mapa con un `for` — sin usar Mongo. Si no se hizo ese ejercicio opcional, alcanza con `MongoRepository`.
...
**Evalúa:** operadores de comparación de Mongo (`$gte`/`$lte`) combinados condicionalmente, y que extender la interfaz otra vez obliga a actualizar **todas** sus implementaciones existentes, reforzando el Ejercicio 4 de la Clase 6.
...
La paginación de la Clase 5 era manual, sobre un slice completo ya en memoria. Acá se pagina **en la base**, sin traer todo.
...
**Evalúa:** que paginar "de verdad" implica dos llamadas al driver (una para los datos, otra para el conteo), y no traer todo a memoria para después recortar un slice como en la Clase 5 — la diferencia entre paginación en la aplicación y paginación en la base.
...
Reemplazar la validación manual de nombre duplicado (Clase 6) por una garantía real de la base.
...
3. Quitar (o dejar como comentario explicando por qué ya no hace falta) la validación manual de nombre duplicado que estaba en `Service.Crear` desde la Clase 6 — ahora la garantía la da la base, no el service.
...
Un test que efectivamente habla con la base (no un fake, a diferencia del Ejercicio 4 de la Clase 6).
...
**Evalúa:** diferencia entre un test unitario (Ejercicio 4 de la Clase 6, con fake, rápido, sin dependencias externas) y un test de integración (lento, depende de infraestructura real, pero verifica el comportamiento real del driver), uso de `t.Cleanup` para dejar el entorno de test limpio.
